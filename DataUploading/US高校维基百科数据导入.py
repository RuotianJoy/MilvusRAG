        ...
        try:
            ...
            if len(self.data) > 0:
                ...
                if "vectors" in first_record:
                    logging.debug(
                        f"向量结构: {list(first_record['vectors'].keys()) if isinstance(first_record['vectors'], dict) else '非字典类型'}")
        ...
        for region, colleges in region_data.items():
            ...
            for college in colleges:
                try:
                    ...
                    if len(batch_entities) >= batch_size:
                        try:
                            self.collection.insert(batch_entities, partition_name=region)
                    ...
                    logging.error(f"处理记录 {college.get('id', 'unknown')} 失败: {e}")
                    continue
            ...
            if batch_entities:
                try:
                    self.collection.insert(batch_entities, partition_name=region)
                    total_inserted += len(batch_entities)
                    logging.info(f"已成功插入剩余 {len(batch_entities)} 条 {vector_type} 类型记录")
                except Exception as e:
                    logging.error(f"批量插入剩余记录失败: {e}")
